...
$.fn.heplbox = function( settings ) {
        ...
        var defaultSettings = {
            ...
            settings = $.extend(defaultSettings,settings), // fonction extend
            elements = [], // Tableau des liens 
            ...
            var setup = function()
            {
                ...
                if(! $conteiner.size())
                {
                    $conteiner = $('<div class="'+ settings.CSSPrefix+ 'conteiner"></div>').css({
                        width: 25,
                        height: 25,
                        borderRadius: "5px",
                        position: "absolute",
                        ...
                        background: "white url("+settings.loaderGifURL+") no-repeat center center"
                    }).hide().appendTo("body");
                }
            };

            var elementsClicked = function(e)
            {
                e.preventDefault();
                    ...
                    cursor: "pointer"
                        ...
                        $conteiner.animate({
                                ...
                                top: ($img.height() < $(window).height())?
                                        ($(window).height()-$img.height()-50)/2 : 10
                            },"fast",function(){
                                    // on retire l'image du body
                                    $img.remove().appendTo($conteiner).fadeIn("fast").on("click.heplbox",nextImage);
                        });
                    });
            };
        ...
        return this.each(function()
                {
                    ...
                    $(this).on("click.heplbox",elementsClicked)
                }
            );
    };
